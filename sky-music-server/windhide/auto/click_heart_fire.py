...
from windhide.utils.ocr_screenshot_util import resetGameFrame, get_window_screenshot, \
    match_template_and_return_coordinates
from windhide.utils.path_util import getResourcesPath


def click_heart_fire():
    ...
    # 先判断是不是第一页
    while True:
        friend_position = get_add_friend_position()
    ...
    # 来到第一页
    while True:
        heart_positions = get_heart_fire_position()
        if len(heart_positions) != 0:
            for position in heart_positions:
                ...
                time.sleep(2)
            if can_fire():  # 在点击光崽靠近的时候可能会识别不到，不过在上边已经拖动滚轮了
                key_press("f")
                time.sleep(0.8)
                key_press("ESC")
            # 下一页
            ...
            if len(get_add_friend_position()) == 0:
                key_press("c")
                time.sleep(2.5)
            else:
                return "点火结束"
