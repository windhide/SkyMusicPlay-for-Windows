...
const deleteCurrentColumn=()=>{ notes.value.splice(currentColumn.value, 1); customTexts.value.splice(currentColumn.value, 1); if (currentColumn.value >=notes.value.length){ currentColumn.value=notes.value.length - 1;} progress.value=currentColumn.value + 1; drawCanvas();};
const insertEmptyColumn=()=>{ notes.value.splice(currentColumn.value, 0, []); customTexts.value.splice(currentColumn.value, 0, ['', '', '', '', '', '', '', '', '', '', '', '', '', '', '']); progress.value=currentColumn.value + 1; drawCanvas();};
...
function handleButtonClick(item, column, row) {
  ...
  if (item.active) {
    ...
    customTexts.value[progressIndex][index] = item.duration ? String(item.duration) : "";
  } else {
    notes.value[progressIndex] = notes.value[progressIndex].filter(res => res !== index + 1);
    customTexts.value[progressIndex][index] = "";
  }
